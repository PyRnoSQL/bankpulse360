...
router.get('/performance', requireAuth, async (_req, res) => {
  try {
    const rows = await bqQuery(`
      SELECT * FROM \`${P}.${AN}.vw_branch_performance\`
      ORDER BY Sigma_Level DESC
    `)
    res.json(rows)
...
router.get('/flagged', requireAuth, async (_req, res) => {
  try {
    const rows = await bqQuery(`
      SELECT Branch_ID, Branch_Name, Region, City,
             Sigma_Level, SPC_Flag, sla_compliance,
             latitude, longitude, efficiency_score
      FROM \`${P}.${AN}.vw_branch_performance\`
      WHERE SPC_Flag = 'Out of Control'
         OR Sigma_Level < 2.5
      ORDER BY Sigma_Level ASC
    `)
    res.json(rows)
  } catch (err) { res.status(500).json({ error: err.message }) }
})
